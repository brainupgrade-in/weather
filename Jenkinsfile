pipeline {
    ...
    stages {
        ...
        stage('code checkout') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/monolith']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/brainupgrade-in/weather.git']]])
        ...
        stage('docker build') {
            steps{
                ...
                #!/bin/bash
                cd $WORKSPACE
                docker build -t brainupgrade/weather:jenkins-${BUILD_NUMBER} -f Dockerfile-alpine .
                '''
            }
        }
        ...
        stage('k8s deploy') {
            steps{
                ...
                chmod +x ./kubectl
                sed -i "s/weather:monolith/weather:jenkins-${BUILD_NUMBER}/g" $WORKSPACE/k8s/deploy.yaml
