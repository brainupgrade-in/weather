    ...
    stages {
        ...
        stage('docker build') {
            steps{
                ...
                #!/bin/bash
                cd $WORKSPACE
                docker build -t brainupgrade/weather:features-two-${BUILD_NUMBER} -f Dockerfile .
                '''
            }
        }
        ...
        stage('docker repo push') {
            steps{
                sh(script: """
                    docker push brainupgrade/weather:features-two-${BUILD_NUMBER}
                """)
            }
        }
        ...
        stage('k8s deploy') {
            steps{
                ...
                chmod +x ./kubectl
                sed -i "s/weather:latest/weather:features-two-${BUILD_NUMBER}/g" $WORKSPACE/k8s/deploy.yaml
