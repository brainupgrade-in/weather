pipeline {
    ...
    stages {
        ...
        stage('code checkout') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/microservices-front']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/brainupgrade-in/weather.git']]])
        ...
        stage('k8s deploy') {
            steps{
                ...
                chmod +x ./kubectl
                sed -i "s/weather:microservices-front/weather:jenkins-${BUILD_NUMBER}/g" $WORKSPACE/k8s/deploy.yaml
